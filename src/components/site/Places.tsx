  ...
  return (
    <section id="places" className="relative py-28 md:py-40">
      <div className="mx-auto max-w-7xl px-6">
        <SectionHeader
          eyebrow={t("places_eyebrow")}
          title={
            isNepali ? (
              t("places_title")
            ) : (
              <>
                Sanctuaries of <span className="text-gradient-gold italic">stone & spirit</span>
              </>
            )
          }
          ...
          {items.map((p, i) => (
            ...
            >
              ...
              <div className="absolute inset-x-0 bottom-0 p-7">
                ...
                <p className="mt-2 max-w-md text-sm leading-relaxed text-foreground/75">{p.description}</p>
                <div className="mt-4 inline-flex items-center gap-1.5 text-xs uppercase tracking-[0.25em] text-gold opacity-0 transition-all duration-500 group-hover:translate-y-0 group-hover:opacity-100">
                  {t("places_discover")} <ArrowUpRight className="size-3.5" />
                </div>
              </div>
            </motion.article>
          ))}
